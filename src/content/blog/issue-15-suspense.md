...
`<Suspense>` は、**子コンポーネントの読み込みが完了するまでフォールバック（代替UI）を表示する**ためのReactコンポーネントです。
...
一言でいえば、**「ロード状態の宣言的な管理」** を実現するための仕組みです。
...
// Promiseを作成（コンポーネントの外で1回だけ）
const albumsPromise = fetchAlbums(artistId);
...
### Reactバージョンアップによる挙動変更

Suspenseの挙動は、Reactのバージョンアップに伴って改善・変更されてきた歴史があります。

- React 16.6：`React.lazy` によるコード分割でのみサポート
- React 18：Concurrent Featuresと統合。データフェッチでのSuspense利用が現実的に
- React 19：`use` フックの追加により、Promise読み取りがより直感的に

公式ドキュメントでも、Suspense対応データソースの実装要件は「まだ不安定でありドキュメント化されていない」とされています。規約のあるフレームワーク（Next.js等）を使うか、動向をウォッチしておくことが重要です。
