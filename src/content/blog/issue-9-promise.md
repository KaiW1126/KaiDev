...
特にReact Suspenseの仕組みを理解するには、Promiseの本質が避けて通れません。今回はその土台として、**Promiseの状態遷移と連鎖を「説明できる」レベルにすること**がゴールです。
...
- `async/await` はPromiseの糖衣構文で、`return` = `resolve` / `throw` = `reject` です
...
今回学んだ「Pendingの間は値がない」という性質を、ReactのSuspenseは**UIの出し分け**に活用しています。
